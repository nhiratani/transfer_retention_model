...
def calc_mean_eTF_eRT_wn_mf(params):
	gm = params['gm']
...
def plot_simul_set1(params):
	...
	plt.show()
	svfg1.savefig("fig_tlcf1_lst2_wn_simul_set1_eTF_Nx" + str(params['Nx']) + '_rhoA' + str(params['rhoA']) + '_lr' + str(params['learning_rate'])\
	...
	plt.show()
	svfg2.savefig("fig_tlcf1_lst2_wn_simul_set1_eRT_Nx" + str(params['Nx']) + '_rhoA' + str(params['rhoA']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
...
def plot_simul_set2(params):
	...
	plt.show()
	svfg1.savefig("fig_tlcf1_lst2_wn_simul_set2_eTF_Nx" + str(params['Nx']) + '_rhoB' + str(params['rhoB']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
	...
	plt.show()
	svfg2.savefig("fig_tlcf1_lst2_wn_simul_set2_eRT_Nx" + str(params['Nx']) + '_rhoB' + str(params['rhoB']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
...
def plot_simul_set3(params):
	...
	gmflen = len(gm_mfs)
	gmfones = np.ones((gmflen))
	eTF_mfs = np.zeros((gmflen))
	eRT_mfs = np.zeros((gmflen))
	
	for gmfidx in range(gmflen):
		params['gm'] = gm_mfs[gmfidx]
		eTFtmp, eRTtmp = calc_mean_eTF_eRT_wn_mf(params)
	...
	plt.show()
	svfg1.savefig("fig_tlcf1_lst2_wn_simul_set3_mean_eTF_eTR_Nx" + str(params['Nx']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
					
	svfg2 = plt.figure()
	#plt.errorbar(alphas, eTF_means, eTF_stds[:, raidx], color=clrs[raidx], lw=0.0, elinewidth=2.0, capthick=2.0, capsize=4.0, marker='o', markersize=7.5)
	#plt.axhline( 1/6.0, color='C0', ls='--' )
	plt.plot(gms, perf_means[0], 'o', color='C0')
	plt.plot(gm_mfs, gmfones - np.multiply(gmfones-gm_mfs, gmfones-gm_mfs), '-', color='C0')
	
	#plt.axhline( 1.0 - (1/5.0 - 1/4.0 + 1/3.0), color='C1', ls='--' )
	plt.plot(gms, perf_means[1], 'o', color='C1')
	plt.plot(gm_mfs, gmfones - np.multiply(gmfones-gm_mfs, gmfones-gm_mfs), '-', color='C1')
	#plt.plot(gm_mfs, eRT_mfs, '-', color='C1')
	
	plt.ylim(0.0, 1.025)
	plt.show()
	svfg2.savefig("fig_tlcf1_lst2_wn_simul_set3_mean_task_perf_Nx" + str(params['Nx']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
