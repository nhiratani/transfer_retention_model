...
#
# Context-dependent plasticity gating model
#
...
def calc_eTF_eRT_cpg_mf(params):
...
def calc_mean_eTF_eRT_cpg_mf(params):
	alpha = params['alpha']
	mean_eTF = 1.0/2.0 - 1.0/3.0
	mean_eRT = 1.0 - (1.0/5.0 - 1.0/4.0 + 1.0/3.0)
...
def plot_simul_set1(params):
	...
	for rbidx in range(rBlen):
		...
		for afidx in range(aflen):
			...
			eTFtmp, eRTtmp = calc_eTF_eRT_cpg_mf(params)
	...
	plt.show()
	svfg1.savefig("fig_tlcf1_lst2_cpg_simul_set1_eTF_Nx" + str(params['Nx']) + '_rhoA' + str(params['rhoA']) + '_lr' + str(params['learning_rate'])\
	...
	plt.show()
	svfg2.savefig("fig_tlcf1_lst2_cpg_simul_set1_eRT_Nx" + str(params['Nx']) + '_rhoA' + str(params['rhoA']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
...
def plot_simul_set2(params):
	...
	plt.show()
	svfg1.savefig("fig_tlcf1_lst2_cpg_simul_set2_eTF_Nx" + str(params['Nx']) + '_rhoB' + str(params['rhoB']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
	...
	plt.show()
	svfg2.savefig("fig_tlcf1_lst2_cpg_simul_set2_eRT_Nx" + str(params['Nx']) + '_rhoB' + str(params['rhoB']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
...
def plot_simul_set3(params):
	...
	for afidx in range(aflen):
		params['alpha'] = alpha_mfs[afidx]
		eTFtmp, eRTtmp = calc_mean_eTF_eRT_cpg_mf(params)
	...
	plt.ylim(-0.02, 1.02)
	plt.show()
	svfg1.savefig("fig_tlcf1_lst2_cpg_simul_set3_mean_eTF_eTR_Nx" + str(params['Nx']) + '_lr' + str(params['learning_rate'])\
					+ '_nep' + str(params['num_epochs']) + "_ikm" + str(params['ikmax']) + ".pdf")
