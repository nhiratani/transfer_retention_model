...
def run_exp_wn_cwn(params):
	...
	for t in range( 2*num_epochs ):
		...
		if t < num_epochs:
			...
			W = W - learning_rate*calc_dW_cwn(W, A2, B2, W1, A1, lmbd_c)
...
def run_exp_cwn(params):
	...
	for t in range( 2*num_epochs ):
		if t == num_epochs:
			...
			W1 = W1.at[:,:].set( W[:,:] )
		if t < num_epochs:
			W = W - learning_rate*calc_dW_cwn(W, A1, B1, W0, A0, lmbd_c)
		else:
			W = W - learning_rate*calc_dW_cwn(W, A2, B2, W1, A1, lmbd_c)
		errors[0,t] = fnorm2(B1 - jnp.dot(W, A1))/Ny
		errors[1,t] = fnorm2(B2 - jnp.dot(W, A2))/Ny
...
def simul(params):
	...
	for ik in range(params['ikmax']):
		params['ik'] = ik
		#errs = run_exp_vnl_cwn(params)
		errs = run_exp_cwn(params)
